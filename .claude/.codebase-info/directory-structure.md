...
```
tesseron/
├── packages/                 9 published packages + 2 empty leftovers
...
│   ├── mcp/                  @tesseron/mcp — the gateway, bin tesseron-mcp
│   ├── docs-mcp/             @tesseron/docs-mcp — docs server, bin tesseron-docs-mcp
│   ├── create-tesseron/      EMPTY. no package.json, no source. stale .turbo/ + node_modules/
│   └── devtools/             EMPTY. same.
...
│   └── src/content/docs/     43 pages. docs-mcp bakes these in at build time.
...
By **published artifact**, not by layer or feature. Each `packages/*` directory is one npm package
...
- Error **codes** are in `packages/core/src/protocol.ts:60`, not `errors.ts`.
...
- `packages/vite/tsconfig.json` is the only one that does not extend `tsconfig.base.json`.
- `examples/` is in the pnpm workspace but **excluded from Biome** (`biome.json:8`), so example code
