...
- **Doc blocks carry the why.** The dense files open with a header explaining a constraint rather
  than narrating the code: `packages/core/src/node.ts:7` on why the main entry stays browser-safe,
  `packages/web/src/reactive-core.ts:1` on why the adapters were collapsed,
  `packages/mcp/src/dialer.ts:33` on why `dial()` must be synchronous,
  `packages/core/src/bind-subprotocol.ts:4` on header-over-query-string. Follow that pattern:
...
- **No `Math.random` for anything security-adjacent.** Two tests assert this by scanning source:
  `packages/core/test/claim-mint.test.ts:19` and `packages/mcp/test/session-tokens.test.ts`. Use the
...
- **Constant-time compares for secrets.** `constantTimeEqual` from `@tesseron/core/internal`, with a
  timing test at `packages/core/test/timing-safe.test.ts:65`.
...
- **Internal surface goes through `/internal`**, not the main index. If a sibling package needs it
  and users should not, export it from `packages/core/src/internal.ts`.
